...
```
Now further subset E to only include particle starting location. Set `trimTo` to `FALSE` so we also include particles that leave the starting locations. Because `subsetConnectivity_byPolygon()` comes from the `connectivityUtilities.R`, we must define where the data is by defining `dataDir` and source `connectivityUtilities.R` to load the functions in it. 
``` {r subsetStarts}
