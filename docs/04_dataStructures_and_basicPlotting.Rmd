...
```
Now further subset E to only include particle starting location. Set `trimTo` to `FALSE` so we also include particles that leave the starting locations. Because `subsetConnectivity_byPolygon()` comes from the `connectivityUtilities`, we must source that file to load the functions in it. 
``` {r subsetStarts}
